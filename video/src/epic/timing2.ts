...
export const hook = (() => {
  const narrStart = 0.5;
  const collapseAt = narrStart + nsec("n_hook") + 0.2;
  const len = collapseAt + 1.2;
  return { narrStart, collapseAt, len };
})();
...
export const title = (() => {
  ...
  const nameAt = 0.45;
  const narrStart = 1.0;
  const tagAt = narrStart + nsec("n_title") + 0.2;
  const len = Math.max(6.6, tagAt + 3.0);
  return { barsAt, nameAt, narrStart, tagAt, len };
})();

// ---- Scene 3: voicemail inbox --------------------------------------------
export const inbox = (() => {
  const narrStart = 0.35;
  const cardsAt = [1.0, 1.45, 1.9];
  const memoStart = narrStart + nsec("n_inbox") + 0.45;
  const len = memoStart + ssec("va_memo") + 1.1;
  return { narrStart, cardsAt, memoStart, len };
})();
...
export const live = (() => {
  ...
  const listenAt = 1.3;
  const thinkAt = listenAt + 2.7;
  const prepAt = thinkAt + 2.5;
  const narrEnd = narrStart + nsec("n_live");
  const speakAt = Math.max(prepAt + 1.5, narrEnd + 0.35);
  const len = speakAt + ssec("va_live") + 1.1;
  return { narrStart, composerAt, listenAt, thinkAt, prepAt, speakAt, len };
})();

// ---- Scene 5: two-way send (the centerpiece) -------------------------------
export const twoway = (() => {
  const aStart = 0.25;
  ...
  const replyPrintAt = Math.max(waitingAt + 3.2, bEnd - 1.2);
...
export const trust = (() => {
  const narrStart = 0.35;
  const chipsAt = [0.8, 1.5, 2.2];
  const len = narrStart + nsec("n_trust") + 1.5;
  return { narrStart, chipsAt, len };
})();
