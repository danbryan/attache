#!/usr/bin/env python3
# Targeted regen: only the personalities-scene clips (Sekou intro + the three
# sample voices), now all narrating ONE content-creation update. Leaves every
# other audio clip and its measured duration untouched.
#   ELEVENLABS_API_KEY=... python3 regen-personalities.py
...
# New intro (Sekou narration key in vo-manifest.json)
NARR = {
    "personalities": "And you choose the personality. Here's your latest edit, three ways.",
}
# New sample lines (keys in voice-samples.json) — same event, three personalities
SAMPLES = {
    "vs_explainer":  "I cut your episode into five clips, captioned each one, and queued them to post.",
    "vs_bigpicture": "That one episode is now a full week of shorts, scheduled and ready to go.",
    "vs_inquisitive": "Your clips are cut and captioned. Should I post them daily, or hold for your review?",
}
...
for name, text in SAMPLES.items():
    vs[name]["text"] = text
    vs[name]["seconds"] = save(name, el(text, vs[name]["voice_id"]))
    print(f"sample {name} ({vs[name]['voice']}): {vs[name]['seconds']}s")
