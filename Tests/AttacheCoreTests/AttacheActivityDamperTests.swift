...
final class AttacheActivityDamperTests: XCTestCase {
    ...
    func testRapidToolThinkingBurstDoesNotStrobe() {
        let damper = AttacheActivityDamper(ambientDwell: 1.2, toolKindDwell: 2.0)
        var changes = 0
        var lastPhase: AttacheActivityPhase?
    ...
    func testToolKindStormHoldsTheKind() {
        let damper = AttacheActivityDamper(ambientDwell: 1.2, toolKindDwell: 2.0)
        let kinds: [AttacheToolKind] = [.shell, .read, .edit, .web]
        var kindChanges = 0
        var lastKind: AttacheToolKind?
    ...
    func testBlockedSwitchesInstantlyBothWays() {
        let damper = AttacheActivityDamper()
    ...
    func testMomentShelfLifeIsGenerousEnoughToQueueBehindNarration() {
        XCTAssertGreaterThanOrEqual(AttacheActivityMoment.shelfLife, 5)
    }
}
