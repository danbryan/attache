...
final class SessionSourceRegistryTests: XCTestCase {
    func testProductionRegistryHasExactlyCodexClaudeCodeAndGrokBuildInOrder() {
        let registry = SessionSourceRegistry.production()
        XCTAssertEqual(registry.descriptors.map(\.sourceKind), [.codex, .claudeCode, .grokBuild])
    ...
    func testProductionClassifiesFilesUnderEachSourcesDirectories() {
        ...
        XCTAssertEqual(
            ...
            Set([codexSessions.path, codexArchived.path, claudeProjects.path, grokSessions.path])
        )
    }
