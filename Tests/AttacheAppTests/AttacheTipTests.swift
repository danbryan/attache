...
final class AttacheTipTests: XCTestCase {
    ...
    func testOneTipPerLaunch() {
        let engine = AttacheTipEngine(defaults: freshDefaults())
    ...
    func testTipsNeverRepeatAcrossLaunches() {
        ...
        var seen: Set<String> = []
        for _ in 0..<(AttacheTip.all.count) {
            let engine = AttacheTipEngine(defaults: defaults)
            guard let tip = engine.nextTip() else { return XCTFail("ran dry early") }
            XCTAssertFalse(seen.contains(tip.id), "tip \(tip.id) repeated")
            seen.insert(tip.id)
        }
        let exhausted = AttacheTipEngine(defaults: defaults)
