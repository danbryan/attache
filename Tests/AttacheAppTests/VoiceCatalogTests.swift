...
final class VoiceCatalogTests: XCTestCase {
    private func option(_ id: String, _ name: String) -> AttacheVoiceOption {
        AttacheVoiceOption(id: id, name: name, gender: "female", localeIdentifier: "en_US")
    }
    ...
    func testDetectsNewPremiumVoice() {
        ...
        let found = AttacheVoiceCatalog.newlyAvailableVoice(fresh: fresh, current: current)
    ...
    func testDetectsNewCompactVoiceToo() {
        ...
        XCTAssertEqual(AttacheVoiceCatalog.newlyAvailableVoice(fresh: fresh, current: current)?.name, "Jamie")
    }

    func testPremiumWinsWhenSeveralArriveTogether() {
        let current: [AttacheVoiceOption] = []
        ...
        XCTAssertEqual(AttacheVoiceCatalog.newlyAvailableVoice(fresh: fresh, current: current)?.name,
                       "Ava (Premium)")
    }
}

extension VoiceCatalogTests {
    private func compact(_ name: String, _ locale: String = "en_US") -> AttacheVoiceOption {
        AttacheVoiceOption(id: "com.apple.voice.compact.\(locale).\(name)", name: name, gender: "female", localeIdentifier: locale)
    }
    private func legacy(_ name: String) -> AttacheVoiceOption {
        AttacheVoiceOption(id: "com.apple.speech.synthesis.voice.\(name)", name: name, gender: "male", localeIdentifier: "en_US")
    }
    private func premium(_ name: String, _ locale: String = "en_US") -> AttacheVoiceOption {
        AttacheVoiceOption(id: "com.apple.voice.premium.\(locale).\(name)", name: "\(name) (Premium)", gender: "female", localeIdentifier: locale)
    }
    ...
    func testEnglishSystemGetsHandPickedTrio() {
        ...
        let top = AttacheVoiceCatalog.recommended(from: options, primaryLanguage: "en").prefix(3).map(\.name)
    ...
    func testNonEnglishSystemLeadsWithNativeVoices() {
        ...
        let top = AttacheVoiceCatalog.recommended(from: options, primaryLanguage: "ko").prefix(3).map(\.name)
    ...
    func testDemotedNamesComeAfterNeutralFallbacks() {
        ...
        let top = AttacheVoiceCatalog.recommended(from: options, primaryLanguage: "en").map(\.name)
