...
Runs a headed lifecycle smoke: launch, file an event, quit, relaunch, verify
the window and local event server recover, and open Settings after relaunch.
EOF
}
...
echo "==> Switching Attaché to a fresh lifecycle profile"
...
echo "==> Running Attaché macOS lifecycle UI smoke"
SMOKE_ONLY=f11 \
SMOKE_KEEP_STATE=1 \
ATTACHE_DISABLE_TOPIC_TAGGING=1 \
ATTACHE_LIFECYCLE_NONCE="$NONCE" \
  scripts/ui-smoke.sh

echo "==> macOS lifecycle smoke passed"
