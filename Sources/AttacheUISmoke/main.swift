...
func enabled(_ flow: String) -> Bool {
    ...
    return !["f7", "f8", "f9", "f10", "f11", "f12", "f13", "f14", "f15", "f16", "f17", "f18", "f19"].contains(key)
}
...
if enabled("f17") {
    ...
    run.step("f17-delivery-failure", "environment identifies the disposable Codex session and fake failure text") {
    ...
    run.step("f17-delivery-failure", "Tell Agent stages and confirms an instruction against the fake failing Codex CLI") {
    ...
    run.step("f17-delivery-failure", "delivery failure renders the stderr tail in the conversation status") {
    ...
    run.step("f17-delivery-failure", "two-way log records the failed instruction with the stderr tail") {
        let query = """
        SELECT COUNT(*) FROM instructions \
...
if enabled("f18") {
    ...
    run.step("f18-expiry", "environment identifies the disposable, continuously active Codex session") {
    ...
    run.step("f18-expiry", "Tell Agent stages and confirms an instruction against the never-quiet session") {
        try stageAndConfirmTellAgentInstruction(nonce: nonce, sessionID: sessionID, prompt: prompt)
    }
    ...
    run.step("f18-expiry", "the queued send visibly expires with a message naming the window and target") {
    ...
    run.step("f18-expiry", "two-way log records the expired instruction as failed") {
        let query = """
        SELECT COUNT(*) FROM instructions \
...
if enabled("f19") {
    ...
    run.step("f19-restart-fails-closed", "environment identifies the disposable Codex session") {
    ...
    run.step("f19-restart-fails-closed", "a prior card exists so the Inbox has something to select after relaunch") {
    ...
    run.step("f19-restart-fails-closed", "Tell Agent stages and confirms an instruction against a hung delivery") {
        try stageAndConfirmTellAgentInstruction(nonce: nonce, sessionID: sessionID, prompt: prompt)
    }

    run.step("f19-restart-fails-closed", "the app is terminated and relaunched while the instruction is still in flight") {
    ...
    run.step("f19-restart-fails-closed", "the startup recovery message is visible after relaunch") {
        let button = try waitForElement("voicemail dock button", in: try mainWindow(),
                                        role: kAXButtonRole as String, containing: "Open inbox", timeout: 15)
        guard button.press() else {
            throw SmokeError(message: "AXPress failed on \(button.summary); actions: \(button.actionNames)")
    ...
    run.step("f19-restart-fails-closed", "the interrupted instruction is marked failed in the two-way log") {
        let query = """
        SELECT COUNT(*) FROM instructions \
