...
// MARK: Flow 21: opt-in auto-fallback chain transparently continues a live
...
if enabled("f21") {
    ...
    run.step("f21-conversation-fallback", "environment identifies the two deterministic providers") {
    ...
    run.step("f21-conversation-fallback", "a usage-limit failure is announced and transparently retried on the fallback") {
        try dismissOnboardingIfPresent()
        app.key(Key.l, command: true)
        try selectConversationDestination("Ask Attaché")
        let field = try waitForElement("call message field", in: try mainWindow(),
