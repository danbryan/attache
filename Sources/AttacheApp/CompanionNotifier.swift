...
enum CompanionNotifyScope: String, CaseIterable, Identifiable {
    case needsYouOnly
    case allUpdates
    case off
...
final class CompanionNotifier: NSObject, UNUserNotificationCenterDelegate {
    static let shared = CompanionNotifier()
    ...
    func setApplicationBadgeCount(_ count: Int) {
        UNUserNotificationCenter.current().setBadgeCount(count) { error in
            if let error {
                NSLog("\(CompanionAppSupport.appDisplayName) badge count update failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    func post(card: VoicemailCard, kind: CompanionAlertKind) {
        let center = UNUserNotificationCenter.current()
