...
enum CompanionSecretVault {
    ...
    static func read(account: String) -> String? {
        ...
        guard storesInKeychain else {
            return CompanionDevelopmentSecretStore.read(account: account)
        }
    ...
    static func save(_ value: String, account: String) throws {
        ...
        guard storesInKeychain else {
            try CompanionDevelopmentSecretStore.save(value, account: account)
            return
        }
    ...
    static func delete(account: String) {
        ...
        guard storesInKeychain else {
            CompanionDevelopmentSecretStore.delete(account: account)
            return
        }
        var all = loadUnifiedLocked()
        all.removeValue(forKey: account)
    ...
    private static func persistUnifiedLocked(_ all: [String: String]) throws -> Bool {
        ...
        guard let blob = String(data: data, encoding: .utf8),
              CompanionSecretStore.writeVaultSecret(blob, account: unifiedAccount) else {
    ...
    private static func readUnifiedItem() -> [String: String]? {
        guard let blob = CompanionSecretStore.readVaultSecret(account: unifiedAccount),
    ...
    private static func migrateLegacyKeychainItemsLocked() -> [String: String] {
        ...
        var migrated: [String] = []
        for account in CompanionSecretStore.allVaultAccounts() where account != unifiedAccount {
            if let value = CompanionSecretStore.readVaultSecret(account: account), !value.isEmpty {
                all[account] = value
        ...
        if (try? persistUnifiedLocked(all)) != nil {
            for account in migrated {
                CompanionSecretStore.deleteVaultSecret(account: account)
            }
        }
        return all
    }
