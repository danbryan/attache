...
public enum AttachePersonality {
    ...
    public static func conversationSystemPrompt(
        ...
        let memoryProposalLine = canProposeMemory
            ? "- Call propose_memory ONLY when the user explicitly asks you to remember, save, or note something, and you MUST call it then. An explicit ask includes the user affirming your own offer to remember, or agreeing to save a fact from the turns just before (\"yes, do that\", \"yeah, save it\"). When consent arrives as an affirmation, the fact to restate is the one from the earlier turn, in the words the user used when they stated it. If you offered to remember and the user says yes, you MUST call propose_memory that turn; never offer and then refuse. A spoken acknowledgment alone saves nothing. Never volunteer it for details shared in passing: a name, preference, or fact mentioned without any ask or offer is not a request to remember it. Restate the fact in the user's own words as the statement. The save goes to this Attaché's own memory; if the user asks you to remember something for all of their Attachés, still save it for this Attaché and add one short clause that memories for every Attaché are typed in Settings > Memory.\n- After a save, reply to the person naturally and in character: greet a new name, react to their news, answer what they asked. Do not narrate the save mechanics; Attaché's own UI confirms every save, so at most weave in a brief natural acknowledgment such as \"noted\". Never claim a memory was saved unless the tool reported it. If the tool declines, retry once, restating the fact using the exact words the user spoke, even if ungrammatical; never ask the user to repeat a fact you can already see in this conversation. Only after a failed retry tell them briefly, in one clause, that it can't be saved and why.\n"
