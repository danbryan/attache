...
/// The one semantic activity phase every attache renderer draws from
/// (INF-268). Echoform bars, the Attache character, and any future avatar are all
/// views over this same signal; none of them talk to watchers, playback, or
/// input monitors directly.
public enum AttacheActivityPhase: String, CaseIterable, Codable, Sendable {
    /// No pinned sessions and nothing else happening: the attache can rest.
    case sleeping
...
public enum AttacheAgentIdentity: String, CaseIterable, Codable, Sendable {
    case none
    case codex
    case claude
...
public enum AttacheToolKind: String, CaseIterable, Codable, Sendable {
    case edit
    case read
    case shell
    case web
    ...
    public static func classify(phrase: String, sourceHint: String) -> AttacheToolKind {
...
public struct AttacheFleetSession: Equatable, Sendable, Identifiable {
    ...
    public var agent: AttacheAgentIdentity
...
/// The consolidated state a attache renderer consumes. Semantic fields
...
public struct AttacheActivityState: Equatable, Sendable {
    public var phase: AttacheActivityPhase
    public var activeAgent: AttacheAgentIdentity
    /// Set only while `phase == .toolRunning`.
    public var toolKind: AttacheToolKind?
    ...
    public var fleet: [AttacheFleetSession]
    /// When the FOCUSED session began compacting its context (PreCompact), or
    /// nil. The character squishes gradually toward full while this holds and springs
    /// back when it clears (PostCompact or a focus change). Focus-tied on
    /// purpose: the character only reacts to the session the user is watching.
    public var compactingSince: Date?

    public init(
        phase: AttacheActivityPhase = .sleeping,
        activeAgent: AttacheAgentIdentity = .none,
        toolKind: AttacheToolKind? = nil,
        audio: VisualizerRenderState = VisualizerRenderState(),
        userTyping: Bool = false,
        unreadCount: Int = 0,
        hasCards: Bool = false,
        fleet: [AttacheFleetSession] = [],
    ...
    public static let initial = AttacheActivityState()

    /// The same semantic state with a fresh audio frame, for per-frame
    /// composition inside a renderer.
    public func with(audio: VisualizerRenderState) -> AttacheActivityState {
        var next = self
        next.audio = audio
        return next
    }
...
/// A plain-value snapshot of the signals `AttacheActivityState.derive(from:)`
...
/// wins.
public struct AttacheActivitySignals: Equatable, Sendable {
    ...
    public var speakingAgent: AttacheAgentIdentity?
    ...
    public init(
        ...
        blockedAgent: AttacheAgentIdentity? = nil,
        erroredAgent: AttacheAgentIdentity? = nil,
        workingAgent: AttacheAgentIdentity? = nil,
        respondingAgent: AttacheAgentIdentity? = nil,
        toolAgent: AttacheAgentIdentity? = nil,
        toolKind: AttacheToolKind? = nil,
        playbackIsPlaying: Bool = false,
        playbackIsPaused: Bool = false,
        speakingAgent: AttacheAgentIdentity? = nil,
...
public struct AttacheActivityMoment: Equatable, Identifiable, Sendable {
    public enum Kind: String, CaseIterable, Codable, Sendable {
        ...
        /// alert perk, then the character moves on. The lasting reminder is the pinned
    ...
    public init(id: UUID = UUID(), kind: Kind, agent: AttacheAgentIdentity, at: Date) {
        self.id = id
        self.kind = kind
        self.agent = agent
        self.at = at
...
public final class AttacheActivityDamper {
    ...
    public func damp(_ proposed: AttacheActivityState, now: Date) -> AttacheActivityState {
...
extension AttacheActivityState {
    /// Pure reducer from a signal snapshot to the state renderers show.
    ///
    /// A session that needs the user is deliberately NOT a phase here. The character
    ...
    public static func derive(
        from signals: AttacheActivitySignals,
        audio: VisualizerRenderState = VisualizerRenderState()
    ) -> AttacheActivityState {
        let ambient = { (phase: AttacheActivityPhase, agent: AttacheAgentIdentity) in
            AttacheActivityState(
                phase: phase,
                activeAgent: agent,
