...
| **Grok Build CLI** | `grok --resume <id> --output-format json -p "<text>"` | Resume-by-id appends real turns to the same session's `chat_history.jsonl` under `~/.grok/sessions/<encoded-project>/<id>/`. `--resume`, `-p/--single`, and `--output-format` are TOP-LEVEL grok flags (INF-394); `grok agent --resume` is rejected. Resumes relative to the session's original cwd, like Claude Code. | headless resume, queued until idle |
| **opencode CLI** | `opencode run --session <id> --format json "<text>"` | Resume-by-id appends the user turn and the assistant reply as new `message`/`part` rows in the shared SQLite database (`$XDG_DATA_HOME/opencode/opencode.db`), not a per-session file (INF-395). Readiness (latest message is a `finish == "stop"` assistant turn) and positional reply correlation (first completed assistant turn after the delivery's message-time checkpoint) run over those rows via the SQLite two-way seam. Never pass `-m/--model` (session/config decide) nor `-p` (that is `--password`). | headless resume, queued until idle |
