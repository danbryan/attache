...
| `metadata`             | object of string to string | **required key** | Free-form key/value strings. The key must be present; its value may be `{}`. Two entries are read specially if present, letting your integration control narration precisely instead of relying on Attaché's automatic summarizer: `companion_summary` (or legacy `card_summary`) overrides the card's short summary, and `companion_spoken_text` (or legacy `spoken_text`) overrides what gets spoken. Every other metadata key is stored but not otherwise interpreted by Attaché for a third-party source. |
